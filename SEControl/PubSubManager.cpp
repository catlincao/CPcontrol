...
bool PubSubManager::Subscribe( const char *channel )
{
	...
	cmd.Format(_T("%s %s"), op, channel);
//	DEBUGLOG(cmd);

	return (REDIS_OK == redisAsyncCommand(redisCtx_, RedisCallback, this, str2char(cmd)));
}
...
bool PubSubManager::Publish( const char *channel, const char *message )
{
	...
	cmd.Format(_T("PUBLISH %s %s"), channel, message);
	//DEBUGLOG(cmd);
